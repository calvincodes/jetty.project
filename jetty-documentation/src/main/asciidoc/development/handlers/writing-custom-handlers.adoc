...
The `Handler` needs to be added to the server classpath as described link:https://www.eclipse.org/jetty/documentation/current/startup-classpath.html[here].
Then it can be added to the server, either by overriding some existing XML configuration files such as jetty.xml as shown below, or by defining a custom module as described link:https://www.eclipse.org/jetty/documentation/current/custom-modules.html[here].
